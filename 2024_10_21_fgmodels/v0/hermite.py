...
def model(z=None, nz=None, mn=None, cov=None, mn_pars=None, zbins=None):
    ...
    for i in range(4):
        ...
        params[f"mu_b{i}"] = numpyro.sample(f"mu_b{i}", dist.Uniform(0.3, 1.5))
        params[f"sigma_b{i}"] = numpyro.sample(f"sigma_b{i}", dist.LogUniform(0.2, 1.0))
